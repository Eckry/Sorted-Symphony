...
export const information: Record<Algorithm, AlgorithmInformation> = {
  ...
  MergeSort: {
    description: "Merge Sort is a comparison-based sorting algorithm that follows the Divide and Conquer approach. It divides the input array into two halves, sorts each half separately, and then merges the sorted halves to produce a single sorted array.",
    time: {
      avg: "O(n log n)",
      best: "O(n log n)",
      worst: "O(n log n)",
  ...
  HeapSort: {
    description: "Heap Sort is a comparison-based sorting algorithm that builds a max-heap (or min-heap) from the input array and then repeatedly extracts the maximum (or minimum) element from the heap, ensuring that the heap property is maintained. The extracted elements are placed at the end of the array, effectively sorting it.",
    time: {
      avg: "O(n log n)",
      best: "O(n log n)",
      worst: "O(n log n)",
